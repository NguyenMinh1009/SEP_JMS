...
const EnhancedTableHead: React.FC<EnhancedTableProps> = ({ correlationJobType }) => {
  const currentPerson = useCurrentPerson();
  const getHeaderCells =
    correlationJobType === CorrelationJobType.Job ? jobHeadCells : projectHeadCells;
  return (
    <TableHead className="border-b-0">
      ...
      >
        {(!currentPerson.hiddenPrice
          ? getHeaderCells
          : getHeaderCells.filter(item => item.id !== "price")
        ).map(headCell => (
          <TableCell
            key={headCell.id}
            align="center"
