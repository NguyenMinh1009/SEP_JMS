...
const HomeHero: FC = () => {
  ...
  const [exps, setExps] = useState<Array<Exp>>([]);

  const getReports = async () => {
    // setLoading(true);
    await APIClientInstance.get(`home/infocount`)
      .then(res => {
        console.log(res);
      ...
      <Container maxWidth="lg">
        <Grid container spacing={0} sx={{ flexDirection: { xs: 'column', md: 'unset' } }}>
          ...
          <Grid item xs={12} md={5} sx={{ position: 'relative' }}>
            ...
            <Box sx={{ lineHeight: 0 }}>
              <img src={Images.homeHero} width={775} height={787} alt="Hero img" />
            </Box>
          </Grid>
        </Grid>
        ...
        <Box sx={{ boxShadow: 2, py: 4, px: 7, borderRadius: 4 }}>
          <Grid container spacing={2}>
            {exps.map((item) => (
              <Grid key={JSON.stringify(item)} item xs={12} md={4}>
                <ExpItem item={item} />
              </Grid>
            ))}
          </Grid>
